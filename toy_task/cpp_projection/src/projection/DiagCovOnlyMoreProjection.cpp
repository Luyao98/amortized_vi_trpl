...
vec DiagCovOnlyMoreProjection::forward(double eps, const vec &old_var, const vec &target_var){
    this->eps = eps;
    succ = false;
...
vec DiagCovOnlyMoreProjection::backward(const vec &d_cov) {
...
double DiagCovOnlyMoreProjection::dual(std::vector<double> const &eta_omega, std::vector<double> &grad){
...
vec DiagCovOnlyMoreProjection::last_eta_grad() const {
