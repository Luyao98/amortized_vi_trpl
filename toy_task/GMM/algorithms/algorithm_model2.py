...
def delete_component(model, contexts):
    model.eval()
    with ch.no_grad():
...
def add_components(model, target, contexts):
    ...
    with ch.no_grad():
        ...
        samples = model.get_samples_gmm(current_gate[:,:idx], current_mean[:,:idx], current_chol[:,:idx], 30)  # (b,s,f)
    ...
    return idx
def train_new_component(model, target, contexts, new_component_index, n_epochs=20):
    model.train()
    ########
    # model.mode = True
    ########
    optimizer_add = optim.Adam([
        ...
        {'params': model.gaussian_list.fc_mean.weight, 'lr': 0.01},
    ...
    for epoch in range(n_epochs):
        ...
        # loss = ch.sum(log_model - log_target - aux_loss)+ ch.sum((mean_pred[:,:new_component_index]-mean_init[:,:new_component_index]) **2)
        ...
        # model.gaussian_list.fc_mean.weight.grad[:new_component_index * model.dim] = 0
        # model.gaussian_list.fc_mean.bias.grad[:new_component_index * model.dim] = 0
        # model.gaussian_list.fc_chol.weight.grad[:new_component_index * (model.dim * (model.dim + 1) // 2)] = 0
        # model.gaussian_list.fc_chol.bias.grad[:new_component_index * (model.dim * (model.dim + 1) // 2)] = 0
...
def train_model(model: ConditionalGMM or ConditionalGMM2 or ConditionalGMM3,
    ...
    for epoch in range(n_epochs):
        ...
        # model.eval()
        # with ch.no_grad():
        n_adds = 0.6 * n_epochs // 6
        if epoch < 0.6 * n_epochs and (epoch + 1) % n_adds == 0 and model.active_components < n_components:
            delete_component(model, shuffled_contexts[0:batch_size])
