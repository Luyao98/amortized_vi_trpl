...
from toy_task.GMM.algorithms.evaluation.JensenShannon_Div import js_divergence, js_divergence_gate
...
def train_model_p(model: ConditionalGMM or ConditionalGMM2 or ConditionalGMM3,
    ...
    for epoch in range(n_epochs):
        ...
        for batch_idx in range(0, train_size, batch_size):
            ...
            # projection step
            gate_proj = get_optimal_p_batch(ch.exp(gate_pred), ch.exp(b_gate_old).clone().detach(), eps_gate)
            if proj_type=="w2":
                b_sqrt_old = b_chol_old @ b_chol_old.transpose(-1, -2)
                sqrt_pred = chol_pred @ chol_pred.transpose(-1, -2)
                cov_pred = sqrt_pred @ sqrt_pred
                mean_proj = []
                cov_proj = []
                for j in range(n_components):
                    mean_proj_j, sqrt_proj_j = split_w2_projection(mean_pred[:, j], sqrt_pred[:, j],
                                                                   b_mean_old[:, j].clone().detach(),
                                                                   b_sqrt_old[:, j].clone().detach(),
                                                                   eps_mean, eps_cov)
                    mean_proj.append(mean_proj_j)
                    cov_proj.append(sqrt_proj_j @ sqrt_proj_j)
                mean_proj = ch.stack(mean_proj, dim=1)
                cov_proj = ch.stack(cov_proj, dim=1)
            else:
                mean_proj = []
                chol_proj = []
                for j in range(n_components):
            ...
            for j in range(n_components):
                ...
                if proj_type == "kl":
                    ...
                    cov_proj_j = cov_proj[:, j]

                    # target and target log probability
                    model_samples = model.get_rsamples_w2(mean_proj_j, cov_proj_j, n_samples)
                    log_model_j = model.log_prob_w2(mean_proj_j, cov_proj_j, model_samples)
                    log_target_j = target.log_prob_tgt(b_contexts, model_samples)

                    # regression step
                    pred_dist = MultivariateNormal(mean_pred_j, cov_pred_j)
                    proj_dist = MultivariateNormal(mean_proj_j.clone().detach(), cov_proj_j.clone().detach())
                    reg_loss = kl_divergence(proj_dist, pred_dist).unsqueeze(1).expand(-1, model_samples.shape[1])

                    aux_loss = model.auxiliary_reward_w2(j, gate_pred.clone().detach(), mean_proj.clone().detach(),
                                                      cov_proj.clone().detach(), model_samples)
        ...
        if (epoch + 1) % n_plot == 0:
            ...
            model.train()
            wandb.log({"Jensen Shannon Divergence between gates": js_divergence_gates.item(),
